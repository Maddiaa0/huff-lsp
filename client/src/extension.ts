...
export async function activate(context: ExtensionContext) {
  let disposable = commands.registerCommand(
    ...
    async (uri) => {
      ...
      const url = Uri.parse(
        "/home/sean/Documents/self/rust/lsp/huff-lsp/examples/index.huff"
      );
  ...
  const traceOutputChannel = window.createOutputChannel(
    "Huff Language Server trace"
  );
  console.log(process.env.SERVER_PATH);
  const command = process.env.SERVER_PATH || "huff-language-server";
  ...
  let clientOptions: LanguageClientOptions = {
    ...
    documentSelector: [{ scheme: "file", language: "huff" }],
    synchronize: {
      // Notify the server about file changes to '.clientrc files contained in the workspace
      fileEvents: workspace.createFileSystemWatcher("**/.huff"),
    },
    traceOutputChannel,
  };
